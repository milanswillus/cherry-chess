...
class GameViewModel: ObservableObject {
    ...
    /// Moves are still analyzed in the background so the game can build the
    /// player's rating & accuracy stats.
    ...
    func capturedPieces(for color: Piece.Color, on board: Board) -> [Piece.Kind] {
        ...
        return captured.sorted { PieceValues.material(of: $0) > PieceValues.material(of: $1) }
    }
    ...
    func materialScore(for color: Piece.Color, on board: Board) -> Int {
        ...
        for sq in Square.allCases {
            if let piece = board.position.piece(at: sq) {
                let val = PieceValues.material(of: piece.kind)
