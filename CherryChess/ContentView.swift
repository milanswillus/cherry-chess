...
struct ContentView: View {
    ...
    /// The player's chosen name (from onboarding/settings), falling back to a generic label.
    ...
    var body: some View {
        ...
        ZStack(alignment: .bottom) {
            ...
            ZStack {
                ...
                ProfileView(isActive: selectedTab == 4)
        ...
        .onChange(of: viewModel.gameOver) { oldValue, isOver in
            if isOver && !gameSaved {
                ...
                let game = SavedGame(
                    ...
                    accuracy: viewModel.accuracy(for: viewModel.playerColor, upTo: viewModel.history.count - 1),
                    isChallenge: viewModel.isChallengeMode,
                    counts: counts
                )
            ...
            if !isGameActive {
                ...
                GeometryReader { screenGeo in
                    ...
                    HStack(spacing: isIPad ? 12 : 6) {
                        ...
                        // Eval bar toggle — hidden in Challenge mode (no assistance)
                        if !viewModel.isChallengeMode {
                    ...
                    // Opening name (own full-width row so it never truncates)
                    let dispOpeningName = currentOpeningName ?? (appLanguage == "de" ? "Partie" : "Game")
                    Text(dispOpeningName)
                        .font(.roundedSystem(isIPad ? .title3 : .subheadline, weight: .bold))
                    ...
                    VStack(spacing: 4) {
                        PlayerProfileView(
                            name: viewModel.isFriendMode ?
                                ...
                                (viewModel.isChallengeMode
                                    ? (botName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Stockfish" : botName)
                            ...
                            rating: (viewModel.isFriendMode || viewModel.isChallengeMode) ? "" : (analyzer.currentElo >= 3190 ? "Max" : "\(analyzer.currentElo)"),
                    ...
                    HStack(spacing: 8) {
                        ChessBoardView(
                            ...
                            showAnalysis: showAnalysis && !viewModel.isChallengeMode,
                            showClassificationBadge: showClassificationBadge && !viewModel.isChallengeMode,
                            displayClassification: viewModel.isChallengeMode ? nil : displayClassification
                        )
                        .frame(width: boardWidth, height: boardWidth)

                        if showEvalBar && !viewModel.isChallengeMode {
                    ...
                    VStack(spacing: 4) {
                        PlayerProfileView(
                            name: viewModel.isFriendMode ?
                                ...
                                playerDisplayName,
                            rating: (viewModel.isFriendMode || viewModel.isChallengeMode) ? "" : "\(viewModel.displayLiveElo)",
                    ...
                    Group {
                        ...
                        } else if !viewModel.isFriendMode && !viewModel.isChallengeMode {
                            HStack {
    ...
    private func processMoveSequence() async {
        ...
        if let lastMove = viewModel.lastMove, let fenBefore = viewModel.lastPlayerMoveFenBefore {
            ...
            if showAnalysis || viewModel.isChallengeMode {
                ...
                await MainActor.run {
                    withAnimation {
                        lastClassification = viewModel.isChallengeMode ? nil : classification
                        showClassificationBadge = !viewModel.isChallengeMode
                    }
        ...
        if viewModel.isEngineTurn && !viewModel.gameOver {
            ...
            if let result = engineResult,
               let move = EngineLANParser.parse(move: result.bestMove, for: viewModel.engineColor, in: viewModel.board.position) {
                ...
                if (showAnalysis || viewModel.isChallengeMode), let engineMove = viewModel.lastMove {
                    analyzer.debugStatus = "Analysiere gegnerischen Zug..."
                    
                    let engineEvalBefore = evalAfter
                    let postEngineResult: (bestMove: String, score: Int, mate: Int?)?
                    if case .checkmate = viewModel.board.state {
                        postEngineResult = (bestMove: "", score: -10000, mate: nil)
                    } else if viewModel.gameOver {
                    ...
                    await MainActor.run {
                        withAnimation {
                            lastClassification = viewModel.isChallengeMode ? nil : engineClass
                            showClassificationBadge = !viewModel.isChallengeMode
                        }
            ...
            if !isAnalysisGameActive {
                ...
                GeometryReader { screenGeo in
                    ...
                    VStack(spacing: 8) {
                        ...
                        // Opening name (own full-width row so it never truncates)
                        let dispAnalysisOpeningName = analysisCurrentOpeningName ?? (appLanguage == "de" ? "Partie" : "Game")
                        Text(dispAnalysisOpeningName)
                            .font(.roundedSystem(isIPad ? .title3 : .subheadline, weight: .bold))
                            .foregroundColor(Theme.accentColor)
                            .lineLimit(1)
                            .minimumScaleFactor(0.6)
        ...
        return ZStack(alignment: .top) {
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ...
                    // Start Game button (kept at the top so it is reachable without scrolling)
                    Button(action: {
                        viewModel.isFriendMode = false
                        // Challenge mode disables all assistance regardless of the toggles above.
                        viewModel.allowHints = challengeMode ? false : allowHints
                        viewModel.showBestMovesRetrospectively = challengeMode ? false : showBestMovesRetrospectively

                        analyzer.updateElo(Int(selectedElo))
                        evaluationCache.removeAll()
                        gameSaved = false
                        viewModel.startGame(timed: isTimed, durationSeconds: Int(timeControlMinutes * 60), incrementSeconds: Int(timeControlIncrementSeconds))
                        viewModel.isChallengeMode = challengeMode
                        if challengeMode { showEvalBar = false }
                        lastClassification = nil
                        showClassificationBadge = false
                        resetHints()
                        isGameActive = true
                    ...
                    Button(action: {
                        ...
                        HStack(spacing: 14) {
                            Image(systemName: challengeMode ? "flag.checkered" : "flag.checkered")
                                .font(.system(size: 26, weight: .bold))
                                .foregroundStyle(challengeMode ? AnyShapeStyle(Theme.accentColor) : AnyShapeStyle(Theme.textSecondary))
                                .frame(width: 40)

                            VStack(alignment: .leading, spacing: 4) {
                                Text(L10n.tr("challenge_mode"))
                                    .font(.roundedSystem(.headline, weight: .bold))
                                    .foregroundColor(Theme.textMain)
                                Text(L10n.tr("challenge_mode_desc"))
                            ...
                            Image(systemName: challengeMode ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 24, weight: .bold))
                                .foregroundColor(challengeMode ? Theme.accentColor : Theme.textSecondary.opacity(0.5))
                        }
                        .padding(16)
                        .background(challengeMode ? Theme.accentColor.opacity(0.12) : Theme.panelBackground)
                        .cornerRadius(16)
                        .overlay(
                            RoundedRectangle(cornerRadius: 16)
                        ...
                        if challengeMode {
                            HStack(spacing: 8) {
                                Image(systemName: "lock.fill")
                                    .foregroundColor(Theme.textSecondary)
                                Text(L10n.tr("challenge_assist_locked"))
                                    .font(.roundedSystem(.subheadline, weight: .bold))
                                    .foregroundColor(Theme.textSecondary)
                                    .fixedSize(horizontal: false, vertical: true)
                                Spacer()
    ...
    var customTabBar: some View {
        ...
        return HStack(spacing: 0) {
            ...
            tabButton(index: 4, title: L10n.tr("profile"), systemImage: "person.crop.circle.fill")
        }
...
struct SettingsView: View {
    ...
    @State private var eloValue: Double = 800
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ...
                    // Player Profile Section (Name + Chess.com Elo)
                    VStack(alignment: .leading, spacing: 12) {
                        Text(L10n.tr("your_profile"))
                            .font(.headline)
                            .foregroundColor(Theme.textMain)
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            // Name
                            VStack(alignment: .leading, spacing: 8) {
                                Text(L10n.tr("onboarding_name_title"))
                                    .font(.caption.bold())
                                    .foregroundColor(Theme.textSecondary)
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                                    Text(L10n.tr("chesscom_rating"))
                                        .font(.caption.bold())
                                        .foregroundColor(Theme.textSecondary)
                                    Spacer()
                                    Text(eloKnown ? "\(Int(eloValue))" : L10n.tr("profile_elo_unknown"))
                                        .font(.roundedSystem(.subheadline, weight: .bold))
                                        .foregroundColor(Theme.accentColor)
                                }
                                if eloKnown {
                                    Slider(value: $eloValue, in: 100...3000, step: 10)
                                        .tint(Theme.accentColor)
                                        .onChange(of: eloValue) { _, newVal in
                                            playerChessComElo = Int(newVal)
                                        }
                                }
                                Toggle(L10n.tr("onboarding_elo_unknown"), isOn: Binding(
                                    get: { !eloKnown },
                                    set: { unknown in
                                        eloKnown = !unknown
                                        playerChessComElo = eloKnown ? Int(eloValue) : 0
                                    }
                                ))
                                .toggleStyle(ThemeToggleStyle())
                            }
        ...
        .onAppear {
            if playerChessComElo > 0 {
                eloKnown = true
                eloValue = Double(playerChessComElo)
            } else {
                eloKnown = false
            }
        }
